...
mock.onGet("/api/calendar-app/events")
    .reply((config) => {
	    return [200, calendarDB.events]
    })

mock.onPost("/api/calendar-app/add-event")
    .reply((request) => {
...
mock.onPost("/api/calendar-app/update-event")
    .reply((request) => {
	    const data = JSON.parse(request.data)
...
mock.onPost("/api/calendar-app/remove-event")
    .reply((request) => {
	    const data = JSON.parse(request.data)
