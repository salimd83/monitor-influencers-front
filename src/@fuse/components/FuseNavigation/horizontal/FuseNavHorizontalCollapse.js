...
class FuseNavHorizontalCollapse extends Component {
    ...
    render() {
        ...
        return (<ul className={classNames(classes.root, 'relative pl-0')}>
            <Manager>
                <Target>
                    <ListItem
                        button
                            ...
                            {item.children && (<ul className={classNames(classes.children, 'pl-0')}>
                                {item.children.map((item) => (
                                    ...
                                    <React.Fragment key={item.id}>
                                        ...
                                        {item.type === 'group' && (<FuseNavHorizontalGroup item={item}
                                                                                           nestedLevel={nestedLevel + 1}/>)}

                                        {item.type === 'collapse' && (<NavHorizontalCollapse item={item}
                                                                                             nestedLevel={nestedLevel + 1}/>)}

                                        {item.type === 'item' && (<FuseNavHorizontalItem item={item}
                                                                                         nestedLevel={nestedLevel + 1}/>)}
