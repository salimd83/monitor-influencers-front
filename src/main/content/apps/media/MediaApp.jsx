...
export class MediaApp extends Component {
    handleClick  = () => {
        ...
        const strFrom                                   = moment(from)
            .toISOString()
        const strTo                                     = moment(to)
            .toISOString()
        const profileId                                 = typeof profile.id !== 'undefined' && profile.id !== ''
                                                          ? profile.id : '*'
    ...
    loadNextPage = isVisible => {
        ...
        if (isVisible && page) {
            const strFrom   = match.params.from || moment(from)
                .toISOString()
            const strTo     = match.params.to || moment(to)
                .toISOString()
