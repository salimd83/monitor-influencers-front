...
class LinkDialog extends Component {
  ...
  closeComposeDialog = () => {
    this.props.linkDialog.type === "edit"
      ? this.props.closeEditLinkDialog()
      : this.props.closeNewLinkDialog();
  };
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (!prevProps.linkDialog.props.open && this.props.linkDialog.props.open) {
      ...
      if (
        this.props.linkDialog.type === "edit" &&
        this.props.linkDialog.data &&
        !_.isEqual(this.props.linkDialog.data, prevState)
      ) {
  ...
  render() {
    const { classes, linkDialog, addLink, updateLink, removeLink, profileId } = this.props;
    const { title, type, value } = this.state;
    const typeList = [
      "facebook",
      "instagram",
        ...
        <DialogContent classes={{ root: "p-24" }}>
          <div className="flex">
            ...
            <FormControl className={classes.formControl} required fullWidth>
              ...
              <Input autoFocus id="title" name="title" value={title} onChange={this.handleChange} />
            </FormControl>
          </div>
