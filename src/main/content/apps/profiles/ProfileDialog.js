...
class ProfileDialog extends Component {
    ...
    validate           = ({
                              name, type = '', required = false, min = false, max = false
                          }) => {
        const errors = {}
    ...
    closeComposeDialog = () => {
        this.props.profileDialog.type === 'edit' ? this.props.closeEditProfileDialog()
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
         ...
         * After Dialog Open
         */
        if (!prevProps.profileDialog.props.open && this.props.profileDialog.props.open) {
            /**
             * Dialog type: 'edit'
             * Update State
             */
            if (this.props.profileDialog.type === 'edit' && this.props.profileDialog.data && !_.isEqual(this.props.profileDialog.data, prevState)) {
                this.setState({
             ...
             * Update State
             */
            if (this.props.profileDialog.type === 'new' && !_.isEqual(newProfileState, prevState)) {
                this.setState({
                                  ...this.state,
    ...
    render() {
        const {
                  classes, profileDialog, addProfile, updateProfile, industries, countries, categories, addingProfile, addedProfile, addedProfileId, resetAddProfile, errors
              } = this.props

        let actionButtons
        if (profileDialog.type === 'new') {
            if (addedProfile) {
                actionButtons = (<div className={classes.wrapper} justify="center">
                    <Button
                        variant="raised"
                        color="primary"
                ...
                actionButtons = (<div className={classes.wrapper}>
                    <Button
                        variant="raised"
                        color="primary"
                        onClick={() => {
                    ...
                    {addingProfile && (<CircularProgress size={24} className={classes.buttonProgress}/>)}
                </div>)
            }
        }
        else {
            actionButtons = (<Button
                ...
                color="primary"
                onClick={() => {
                    updateProfile(_.omit(this.state, [
                        'errors',
                        'tags',
                        'active',
                        'profile_picture'
        ...
        >
            ...
            >
                ...
                <div className="flex flex-col items-center justify-center pb-24">
                    <Avatar
                        className="w-96 h-96"
                        alt="profile avatar"
                        src={this.state.profile_picture || 'assets/images/avatars/profile.jpg'}
                    />
                    {profileDialog.type === 'edit' && (<Typography variant="title" color="inherit" className="pt-8">
                        {this.state.name}
                    </Typography>)}
                </div>
            </AppBar>
            ...
            <DialogContent classes={{root: 'p-24'}}>
                ...
                />))}
                <br/>
                {addedProfile ? (<FuseAnimate animation="transition.expandIn" delay={300}>
                    <div align="center">
                        <Icon
                            className="mt-8"
                            style={{fontSize: 36}}
...
export default withStyles(styles, {withTheme: true})(connect(mapStateToProps, mapDispatchToProps)(ProfileDialog))
