    ...
    return (
      <Card className={classNames(classes.root, "w-full", "h-full")}>
        ...
        <table className="simple clickable">
          ...
          <tbody>
            {data.rows.length === 0 ? <tr><td>No Data To show</td></tr> : data.rows.map(row => (
              <tr key={row.name}>
